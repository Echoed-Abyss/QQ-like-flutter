...
class ApiService {
  ...
  Future<ApiResponse<List<GroupInfo>>> getGroupList() async {
    return _get<List<GroupInfo>>(
      '/group/list',
      dataParser: (data) {
        List list = data as List;
  ...
  Future<ApiResponse<List<FriendGroup>>> getFriendList() async {
    return _get<List<FriendGroup>>(
      '/user/friends',
      dataParser: (data) {
        List list = data as List;
        return list.map((e) => FriendGroup.fromJson(e)).toList();
      },
    );
  }

  Future<ApiResponse<Map<String, dynamic>>> createGroup(
      String name, String description, {String? avatar, List<int>? memberIds}) async {
