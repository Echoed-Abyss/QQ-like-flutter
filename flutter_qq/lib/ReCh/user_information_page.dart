...
class _UserInformationPageState extends State<UserInformationPage>
    ...
    return GestureDetector(
      onTap: () {
        if (avatarUrl != null && avatarUrl.isNotEmpty) {
          ...
          _showImageDialog("assets/images/ro_icon.png", isNetwork: false);
        }
      },
      child: Hero(
  ...
  Widget _buildFavoriteButton() {
    return Column(
      ...
      children: [
        GestureDetector(
          onTap: _isCurrentUser ? null : () => _likeUser(),
          child: Container(
            width: 36,
            height: 36,
            decoration: BoxDecoration(
  ...
  Future<void> _likeUser() async {
    ...
    try {
      final response = await ApiService().likeUser(_user!.id);
      if (response.isSuccess) {
        setState(() {
          _user = UserModel(
            id: _user!.id,
            rechNumber: _user!.rechNumber,
