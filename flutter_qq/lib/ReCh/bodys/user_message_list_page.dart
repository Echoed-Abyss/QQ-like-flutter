...
class _UserMessageListPageState extends State<UserMessageListPage>
  ...
  Future<void> initData() async {
    ...
    try {
      ...
      final groupResponse = await ApiService().getGroupList();
      if (groupResponse.isSuccess && groupResponse.data != null) {
        for (var group in groupResponse.data!) {
          messages.add(MsgModel(
            imageurl: group.avatar,
            name: group.name,
            msg: group.lastMsg,
            time: group.lastMsgTime,
            count: group.unreadCount > 0 ? group.unreadCount.toString() : null,
          ));
        }
      }

      final friendResponse = await ApiService().getFriendList();
      if (friendResponse.isSuccess && friendResponse.data != null) {
        for (var friendGroup in friendResponse.data!) {
          for (var friend in friendGroup.friends) {
            messages.add(MsgModel(
              imageurl: friend.avatar,
              name: friend.remark.isNotEmpty ? friend.remark : friend.nickname,
              msg: friend.signature,
              time: '',
              count: null,
            ));
          }
